...
const CampaignOverviewPage: React.FC = () => {
  ...
  const { result: reportApi, loading: reportLoading } = useApiCall<ApiResponse>(
    ...
    //   url: `https://uat-api-dev.mfilterit.net/v1/app/reporting_tool/integrity/summary-table`,
    url: process.env.NEXT_PUBLIC_APP_PERF + `reporting_tool/integrity/summary-table`,
      method: "POST",
      params: {
  ...
  const { result: statusUpdateApi, loading: statusUpdateLoading } = useApiCall<{
    ...
    // url:`https://uat-api-dev.mfilterit.net/v1/app/reporting_tool/integrity/report_status_change`,
    url: process.env.NEXT_PUBLIC_APP_PERF + `reporting_tool/integrity/report_status_change`,
    method: "POST",
    manual: true,
    onSuccess: (response) => {
      setToastData({
  ...
  const { result: deleteApi, loading: deleteLoading } = useApiCall({
    // url: `https://uat-api-dev.mfilterit.net/v1/app/reporting_tool/integrity/delete_report`,
    url: process.env.NEXT_PUBLIC_APP_PERF + `reporting_tool/integrity/delete_report`,
    method: "POST",
    manual: true,
    onSuccess: (response) => {
      setToastData({
  ...
  const handleClone = (item: Record<string, string | number>) => {
    router.push(`/integrity/reportingtool/generate?id=${item.id}&mode=clone`);
  };

  const handleEdit = (item: Record<string, string | number>) => {
    router.push(`/integrity/reportingtool/generate?id=${item.id}&mode=edit`);
  };
  ...
  const adgrpcolumns: Column<AdGrpRowData>[] = [
    ...
    // )},
    {
      ...
      render: (data: AdGrpRowData) => {
        ...
        return (
          <EllipsisTooltip content={formattedDate} />
        );
      }
    },
    {
      ...
      key: "frequency",
      render: (data: AdGrpRowData) => (
        <EllipsisTooltip content={data.frequency || "-"} />
      ),
    },
    {
        title: "Category",
        key: "category",
        render: (data: AdGrpRowData) => (
          <EllipsisTooltip content={data.category || "-"} />
        ),
      },
    { title: "Report Category", key: "report_type" ,render: (data: AdGrpRowData) => (
      <EllipsisTooltip content={data.report_type || "-"} />
    )},
    {
      title: "Report Date",
      key: "report_date",
    ...
    { title: "Report Differentiator", key: "report_differentiator" ,render: (data: AdGrpRowData) => (
      <EllipsisTooltip content={data.report_differentiator || "-"} />
    )},
  ...
  const handleView = (item: Record<string, string | number>) => {
    ...
    router.push(`/integrity/reportingtool/generate?id=${item.id}&mode=view`);
  };
          ...
          <div className="relative bg-card">
            ...
            <div className="p-2">
              <ResizableTable
                ...
                data={tableData as Record<string, string | number>[] ?? []}
                // data={ []}
                isLoading={false}
                isDownload={true}
                headerColor="#DCDCDC"
